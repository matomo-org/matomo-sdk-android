...
public class TrackerBuilder {
    ...
    /**
     * @param apiUrl      Tracking HTTP API endpoint, for example, https://piwik.yourdomain.tld/piwik.php
    ...
    /**
     * Domain used to build the required parameter url (http://developer.piwik.org/api-reference/tracking-api)
    ...
    public Tracker build(Piwik piwik) {
        if (mApplicationBaseUrl == null) {
            mApplicationBaseUrl = String.format("https://%s/", piwik.getContext().getPackageName());
        }
        return new Tracker(piwik, this);
    }
